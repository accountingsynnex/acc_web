...
(function (global) {
  ...
  const Store = {
    ...
    /* The whole workspace as one file, and back again.

       Everything lives in this browser's localStorage and nowhere else: no
       server, no account, no sync. That is the point of the design, and it
       is also the one way to lose a close — clear the browser's site data,
       move to another machine, or mis-click "\u0e25\u0e49\u0e32\u0e07\u0e02\u0e49\u0e2d\u0e21\u0e39\u0e25\u0e17\u0e31\u0e49\u0e07\u0e2b\u0e21\u0e14" and a
       year of imported periods is gone. Exporting per-period statements
       doesn't help: those are the OUTPUT, not the trial balances, journals
       and mappings that produced them.

       So: one file that carries the store verbatim, and a restore that
       checks it is one of ours before overwriting anything. Kept as plain
       JSON rather than anything clever so it can be read, diffed and
       archived alongside the month's workpaper. */
    exportAll() {
      return {
        format: 'fs-close-workspace',
        formatVersion: 1,
        app: (typeof window !== 'undefined' && window.APP_BUILD) || '',
        savedAt: new Date().toISOString(),
        entities: Object.keys(this.data.tb || {}),
        periods: Object.keys(this.data.periods || {}),
        data: this.data,
      };
    },

    /* Read a backup back in. Returns a report rather than throwing so the
       caller can say what happened; nothing is written unless the payload
       really is a workspace export, since restoring garbage over a live
       close would be worse than refusing. `merge` keeps periods this browser
       already has and adds the ones it doesn't — the default replaces
       everything, which is what "restore" normally means. */
    importAll(payload, { merge = false } = {}) {
      const p = payload && payload.format === 'fs-close-workspace' ? payload : null;
      if (!p || !p.data || typeof p.data !== 'object') return { ok: false, error: 'ไม่ใช่ไฟล์สำรองของ FS Close Workspace' };
      if (p.formatVersion > 1) return { ok: false, error: `ไฟล์สำรองเวอร์ชัน ${p.formatVersion} ใหม่กว่าที่โปรแกรมนี้อ่านได้ — อัปเดตเว็บก่อน` };
      const incoming = p.data;
      if (!incoming.tb || typeof incoming.tb !== 'object') return { ok: false, error: 'ไฟล์สำรองไม่มีข้อมูลงบทดลอง' };
      const before = { periods: Object.keys(this.data.periods || {}).length, entities: Object.keys(this.data.tb || {}).length };
      if (merge) {
        const kept = [], added = [];
        for (const [key, period] of Object.entries(incoming.periods || {})) {
          if (this.data.periods[key]) { kept.push(key); continue; }
          this.data.periods[key] = period; added.push(key);
        }
        Object.assign(this.data.mappings, incoming.mappings || {});
        this.data.workbooks = Object.assign({}, incoming.workbooks || {}, this.data.workbooks || {});
        if (!Object.keys(this.data.tb || {}).length) this.data.tb = incoming.tb;
        if (!(this.data.journals || []).length) this.data.journals = incoming.journals || [];
        var report = { merged: true, added, kept };
      } else {
        this.data = incoming;
        report = { merged: false };
      }
      this.load();                                    // fill in anything an older export didn't carry
      // Applying and saving are different failures. No localStorage at all
      // (Node, or a browser with storage switched off) means the restore is
      // live but won't survive a reload — worth saying, not worth calling a
      // failed restore. A quota refusal IS a failed restore: the workspace
      // now holds data the browser can't keep.
      const canPersist = typeof localStorage !== 'undefined';
      const persisted = this.persist();
      const quotaFull = canPersist && !persisted;
      return Object.assign({
        ok: !quotaFull, persisted, before,
        after: { periods: Object.keys(this.data.periods || {}).length, entities: Object.keys(this.data.tb || {}).length },
        savedAt: p.savedAt, app: p.app,
        error: quotaFull ? 'พื้นที่เก็บข้อมูลของเบราว์เซอร์ไม่พอสำหรับไฟล์สำรองนี้' : null,
      }, report);
    },
