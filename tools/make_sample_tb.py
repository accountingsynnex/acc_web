#!/usr/bin/env python3
"""Generate the synthetic trial balances shipped with the app.

These are FABRICATED — no real company figures. They exist so the demo
button and the engine test have something to run on, and so a public
checkout shows a working app instead of empty pages.
...
Writes test/TB_<ENTITY>_<PERIOD>.csv and app/samples.js.
"""
import json
import random
import re
...
TEST_DIR = ROOT / "test"
SAMPLES_JS = ROOT / "app" / "samples.js"
...
def main():
    ...
    for entity, size, share, with_departments in ENTITIES:
        ...
        samples[entity] = {"file": name, "text": text}
        n = len(dept_rows) if dept_rows else len(rows)
        print(f"  {entity:<6} {len(rows):>4} accounts  {n:>5} rows  -> test/{name}")

    header = ("/* Embedded sample trial balances so the demo works offline (file://).\n"
              "   FABRICATED data — generated by tools/make_sample_tb.py, not real figures. */\n")
    SAMPLES_JS.write_text(header + "const SAMPLES = " + json.dumps(samples, ensure_ascii=False) + ";\n",
                          encoding="utf-8")
    print(f"  -> app/samples.js")
    return 0
